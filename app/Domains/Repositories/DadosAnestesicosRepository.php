...
class DadosAnestesicosRepository
{
    ...
    public function getDadosLogbook($matricula)
    {
        ...
        try {
            ...
            $arrayME = [1,2,3];
            $dados = [];
            foreach($arrayME as $indicadorME){

                $listaDadosAnestesicos = $this->model->getDadosLogbook( $matricula, $indicadorME );
                if( count($listaDadosAnestesicos) > 0 ){
                    ...
                    $dados = $this->model->getDadosIndicadorME($matricula, $indicadorME);

                    if (count($dados) > 0) {
                        foreach($dados as $dado){
    
                            $itemsLogbook[$dado->ind_me]['nome'] = 'ME - ' . $dado->ind_me;
                            $itemsLogbook[$dado->ind_me]['inicio'] = \DateTime::createFromFormat('Y-m-d', $dado->dt_inicio_me)->format('d/m/Y');
                            $itemsLogbook[$dado->ind_me]['fim'] = \DateTime::createFromFormat('Y-m-d', $dado->dt_fim_me)->format('d/m/Y');
                            $itemsLogbook[$dado->ind_me]['total'] = 0;
                            $itemsLogbook[$dado->ind_me]['dados'] = Array();
                
                            if( !isset($duracao[$dado->ind_me]) ){
                                $duracao[$dado->ind_me]['nome'] = 'ME - ' . $dado->ind_me;
                                $duracao[$dado->ind_me]['total'] = 0;
                            }
                    
                            if( !isset($atos[$dado->ind_me]) ){
                                $atos[$dado->ind_me]['nome'] = 'ME - ' . $dado->ind_me;
                                $atos[$dado->ind_me]['total'] = 0;
                            }
